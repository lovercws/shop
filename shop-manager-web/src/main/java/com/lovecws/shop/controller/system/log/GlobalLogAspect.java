...
public class GlobalLogAspect {
	...
	public Object globalAround(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		if(sysUser!=null){
			logService.addSysUserLog(new SysUserLog(PublicEnum.NORMAL.value(), SecurityUtils.getSubject().getPrincipal().toString(), new Date(), sysUser.getUserId(), sysUser.getUserName(), SecurityUtils.getSubject().getSession().getHost().toLowerCase(), method, null, null, usetime));
		}
		return proceed;
	}
}
